...
async function _corsBypass(URL) {
...
async function corsBypass(URL) {
 if (!URL.startsWith(window.location.origin)) return await _corsBypass(URL);
...
function getIsDarkMode() {
 if (navigator.userAgent.includes('Dark')) return true;
 if (navigator.userAgent.includes('Pure-Black')) return true;
 if (window.matchMedia && window.matchMedia('(prefers-color-scheme)').media !== 'not all' && window.matchMedia('(prefers-color-scheme: dark)').matches) return true;
 if (getCookie("enableDarkMode")) return true;
}

// Set Dark Mode Cookie if non-existant
if (!document.cookie && !getIsDarkMode()) {
...
function refreshDarkMode() {
 //let darkModeStatus = getCookie("enableDarkMode")
 let darkModeStatus = getIsDarkMode()
